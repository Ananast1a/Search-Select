...
const SearchBar = ({ books }) => {
  ...
  const onChange = (event) => {
    ...
    const foundBooks = books.filter(
      (book) => book.title.toLowerCase().indexOf(userInput.toLowerCase()) > -1
    );
    const foundAuthors = books.filter(
      (book) => book.author.toLowerCase().indexOf(userInput.toLowerCase()) > -1
    );
  ...
  return (
    <div className="search-bar">
      ...
      {showBooks && input && (
        <SearchAutocomplete
          foundByBook={filteredBooks}
          foundByAuthor={filteredAuthors}
          input={input}
