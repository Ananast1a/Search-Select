...
const SearchAutocomplete = ({
  ...
  function getHighlightedText(text) {
    ...
    return (
      <span>
        {parts.map((part) =>
          part.toLowerCase() === input.toLowerCase() ? <b>{part}</b> : part
        )}
      </span>
    );
  }
  const foundBooks = foundByBook.length ? (
    <div className="autocomplete__section">
      <div className="autocomplete__title">Books</div>
          ...
          return (
            ...
            >
              <div className="autocomplete__item-title">
                {getHighlightedText(el.title)}
              </div>
              <div className="autocomplete__item-subtext">
                {el.author} - Published in {el.year}
              </div>
            </div>
          );
        })}
  ...
  const foundAuthors = foundByAuthor.length ? (
    <div className="autocomplete__section">
